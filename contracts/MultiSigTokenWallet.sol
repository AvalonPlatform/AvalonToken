...
contract MultiSigTokenWallet {
    ...
    address constant public TOKEN = <ERC20_CONTRACT_ADDRESS>;

    event Confirmation(address source, uint actionId);
    event Revocation(address source, uint actionId);
    event ActionCreation(uint actionId);
    event ActionDeletion(uint actionID);
    ...
    enum ActionChoices { TransferToken, AddOwner, WithdrawOwner, ChangeQuorum, DeleteAction}
        ...
        if (isConfirmed(actionId)) {
            ...
            if (action.actionType == ActionChoices.AddOwner)
                ...
                if (transferToken(action.addressField, action.value))
                    TokenTransfer(actionId);
                else {    
                    TokenTransferFailure(actionId);
